...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'atom' -CheckDiskSpace -PersistPrompt
		...
		If (Test-Path "$env:Public\Desktop\atom.lnk") {
			Write-Log -Message "Deleting desktop shortcut..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "$env:Public\Desktop\atom.lnk" -Force
		}
		Else {
			Write-Log -Message "Desktop shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		## Delete the start menu shortcut
		If (Test-Path "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\Atom.lnk") {
			Write-Log -Message "Deleting start menu shortcut..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Remove-Item "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\Atom.lnk" -Force
		}
		Else {
			Write-Log -Message "Start menu shortcut not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		## Delete the installation
		If (Test-Path "$envProgramFiles\atom-windows") {
			Write-Log -Message "Deleting installation files..." -Source 'Pre-Installation' -LogType 'CMTrace'
			Get-ChildItem -Path "$envProgramFiles\atom-windows" -Recurse | Remove-Item -force -recurse
			Remove-Item "$envProgramFiles\atom-windows" -Force
		}
		Else {
			Write-Log -Message "Atom installation files not detected." -Source 'Pre-Installation' -LogType 'CMTrace'
		}
		...
		Copy-Item -Path "$dirFiles\atom-windows" -Destination $envProgramFiles -Recurse
		Write-Log -Message "Copying desktop shortcut..." -Source 'Installation' -LogType 'CMTrace'
		Copy-Item -Path "$dirFiles\Atom.lnk" -Destination "$env:Public\Desktop"
		Write-Log -Message "Copying start menu shortcut..." -Source 'Installation' -LogType 'CMTrace'
		Copy-Item -Path "$dirFiles\Atom.lnk" -Destination "$env:ProgramData\Microsoft\Windows\Start Menu\Programs"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'atom' -CloseAppsCountdown 60
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUMQ2D6zlT0YiX3lWiISrWNgQR
# 1AmgghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFHFw4RtWT4FKRTYJrNt5RHXGHj+6MA0GCSqGSIb3DQEBAQUABIIBgCakcVrN
# RCWGnAbS2xh5S9q4Xz9lck5Ite5xQ7alpwx8PR4HTIGA3IzdUgmAq68dxFNaw+CT
# ceu5Ma+5kaDwj0NtXD48L66G3PEMP+Eh3pwyQsOlRFPTeSaS2Zbvwh2nFIQvu8K9
# EVChNIMVzONeOTxtLok5LwOGRX3lxvR/0CTqF2tVNUMOaJHuaAnU06dIx8xQsui0
# 1gSiSIA6DBgLbIplcTjnmqtHYFn3aMAld10ThkOOTssyaalaps2ZDQAO/3Em8RKS
# Xe9rqNkvBwJ7x+74wyl2HAWaFdhviPOo+5sF7NWX/7pZ/kuKM/5Lulrt3xlzUjgv
# a//DmcloeSn9/08fNoZcAEYKpcP2KfWKtJillKE/IRzYr5ItJwsD9NoCQSBUtfYu
# X4L4qmscpYjaARy3sFUK4BQliYkbTRPvcrGOyVpRd8Zw15XKxP98ObD+v41CWdER
# INibZeVJ+IFvFg2S0zZpZa+DhgDuLIsuF86m/b3xBlSdItupkva1rMMlVA==
